      ...
      <el-dropdown @command="handleCommand" class="userInfoAvatar">
        <span class="el-dropdown-link">
          ...
          <img v-if="isLogin&&userInfo.avatar!=undefined" :src="userInfo.avatar"
               onerror="onerror=null;src='https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg'">
          <img v-if="isLogin&&userInfo.avatar==undefined"
               src="https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg">
        </span>
              ...
              <el-card>
                ...
                <span class="left p1">
                  <img v-if="comment.user"
                       :src="comment.user.photoUrl ? comment.user.avatar:'https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg'"
                       onerror="onerror=null;src='https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg'"/>
          ...
          <el-timeline>
            <el-timeline-item v-for="reply in replyList" :key="reply.uid" :timestamp="timeAgo(reply.createTime)"
              ...
              <el-card>
                <div class="commentList">
                  <span class="left p1">
                    <img alt="" v-if="reply.user"
                         :src="reply.user.avatar ? reply.user.avatar:'https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg'"
                         onerror="onerror=null;src='https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg'"/>
                    <img alt="用户头像" v-else src="https://typora-photo.oss-cn-shenzhen.aliyuncs.com/%E5%A4%B4%E5%83%8F/g.jpg"/>
                  </span>
  ...
  methods: {
    ...
    cropSuccess(resData) {
      ...
      if (this.activeName === "0") {
        // 激活个人中心页面
        this.userInfo.photoUrl = resData[0].avatar
        this.userInfo.avatar = resData[0].avatar
    ...
    getToken: function() {
      ...
      if (token !== undefined) {
        // 设置token七天过期
        let expireTime = new Date(new Date().getTime() + 12 * 60 * 60 * 1000);
        setBlogToken(token,expireTime);
      } else {
        // 从cookie中获取token
        token = getBlogToken();
      }
      if (token != undefined) {
        authVerify(token).then(response => {
            this.isLogin = true;
            this.userInfo = response.data.sysUser;
...
.bgImg{
  ...
  background: url('../assets/bg_img1.png') center center no-repeat;
  height: 100%;
  position: fixed;
  width: 100%;
  z-index: -1;
...
body{
  background: url('../assets/bg_img1.png') no-repeat fixed;
